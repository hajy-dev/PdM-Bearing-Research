...
def assign_rul_grade(rul_days: float) -> str:
    """RUL(일) → 등급 (FEMTO/IEEE PHM 2012 기준)"""
    if rul_days <= 2:
        return "CRITICAL"
    elif rul_days <= 15:
        return "HIGH"
    ...
    for b_dir in bearing_dirs:
        ...
        for fp in csv_files:
            try:
                ...
                # 컬럼 구조: [시간, acc_h, acc_v] → acc_h(인덱스 1)
                # 단일 컬럼 파일은 인덱스 0 사용
                vib_col = 1 if n_cols >= 2 else 0
...
# ──────────────────────────────────────────────
# 2-3. XJTU-SY → 고장모드 라벨
#   가정: CSV, 헤더 있음, 컬럼 = ['Horizontal', 'Vertical']
#         폴더명이 고장 유형을 나타냄 (inner/, outer/, ball/, normal/)
# ──────────────────────────────────────────────
XJTU_LABEL_MAP = {           # [수정 필요] 실제 폴더명에 맞게 수정
...
def load_xjtu(xjtu_dir: str) -> pd.DataFrame:
    """
    XJTU-SY 데이터셋 로드
    반환: vibration_rms, motor_current(없으면 0), fault_label 컬럼 포함 DataFrame
    """
    records = []
    ...
    for fp in csv_files:
        ...
        try:
            ...
            elif 'CH1' in df.columns:
                signal = df['CH1'].values.astype(float)
            else:
            ...
            for w in sliding_window(signal):
                records.append({
                    "vibration_rms": compute_rms(w),
                    "motor_current": 0.0,   # XJTU는 motor_current 없음
                    "fault_label"  : label,
                    "source"       : "XJTU",
                })
        except Exception as e:
...
def train_xgboost_rul(df: pd.DataFrame,
    ...
    df = df.dropna(subset=features + ['RUL', 'bearing_id'])
    ...
    # ── 5-6. 등급 정확도
    y_pred_grade  = [assign_rul_grade(v) for v in model_A.predict(X_test)]
    y_true_grade  = [assign_rul_grade(v) for v in y_test]
...
def main():
    ...
    has_temp  = has_kaist   # KAIST에 temperature 실데이터 존재 → 가용 시 True

    # FEMTO + KAIST motor_current/temperature 병합
    if not df_femto.empty:
        if has_kaist:
            df_femto['motor_current'] = df_kaist['motor_current'].mean()
            df_femto['temperature']   = df_kaist['temperature'].mean()
            print(f"  ℹ️  KAIST 평균값 병합 — motor_current: {df_femto['motor_current'].iloc[0]:.3f} / temperature: {df_femto['temperature'].iloc[0]:.3f}")
        else:
    ...
    # ── Step 5. XGBoost RUL 학습
    print("\n[Step 5] XGBoost RUL 회귀 학습")
    if not df_rul.empty and len(df_rul) > 100:
        result_xgb = train_xgboost_rul(df_rul, has_kaist=has_kaist, has_temp=has_temp)
    else:
    ...
    if not df_rul.empty:
        ...
        print(f"  LOW 구간 데이터: {len(df_normal):,} rows")
        if len(df_normal) > 50:
            result_ae = train_autoencoder(df_normal, has_kaist=has_kaist, has_temp=has_temp)
        else:
            print("  ⚠️  정상 데이터 부족")
            result_ae = {}
    else:
