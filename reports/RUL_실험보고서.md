...
- **출력 경로**: `D:\project\예지보전_v2\compare_rul_v2\`
...
- 라벨: rul_ratio 기반 4등급
- 결과: LightGBM Acc **99.99%**, F1 **99.99%**
...
|------|----------|--------|-----|------|------|
| 1 | operating_hours 제거 (4피처) | 29.43% | 19.0% | Leakage 증명 완료 | `증명_RUL_operating_hours제거` |
...
| 7 | balanced weight (compute_sample_weights) | **50.79%** | **3.3%** | **최고 기록** | `증명_RUL_HI라벨_balanced` |
| 8 | 사분위 기반 라벨 기준 실험 | 42.50% | 37.7% | **실패** → 7단계 복원 | `증명_RUL_사분위실패_되돌림` |
| 추가 | XJTU-SY 15 bearings RTF 추가 (22→37) | 46.29% | 8.2% | **실패** (도메인 이질성) | `증명_RUL_XJTU_RTF추가` |
...
- 모든 데이터셋에서 수학적으로 증명, 제거 후 99.99% → ~29%로 하락하여 확인
- 부수 발견: leakage가 환경보정의 진짜 효과도 가리고 있었음 (-0.05%p → +1.75%p 전환)

### HI 기반 라벨링의 효과
- rul_ratio(시간 기반) → health_index(상태 기반)로 전환이 핵심 변곡점
- 피처와 라벨이 같은 물리적 도메인(진동 상태) 공유 → 베어링 간 일반화 성공
- Overfit Gap: 19~28% 수준 → 2~6%로 극적 개선
- Accuracy 47% → 94% 급등 (단, HI 분포 편향 영향 주의)

### 환경보정 효과
- Leakage 있을 때: -0.05%p (효과 없음으로 오인)
- Leakage 제거 후: +1.75%p (1단계), HI 라벨+balanced 시: **+5.95%p** (진짜 효과)

### HI 분포 편향
- 전체 데이터의 98.1%가 LOW(HI > 0.7)
- balanced weight로 소수 클래스 가중치 보강 → RF F1 +17.2%p (가장 극적)
- 근본적 한계: 소수 클래스 절대 부족 (HIGH 200개, CRITICAL 61개)

### min-max vs z-score 정규화
- RTF 데이터는 고장 말기에 극단적 진동값 발생
- min-max: 극값에 의해 정상 구간 정보가 0~0.01 범위로 압축 → 정보 손실
- z-score: 평균/분산 기반으로 극값에 강건 → RTF 데이터에 적합

### XJTU-SY 도메인 이질성
- 운전 조건: 2100~2400rpm, 10~12kN (기존 4개 데이터셋과 이질적)
- LOBO-CV에서 XJTU-SY 베어링이 test fold → train 분포와 달라 예측 불가
- 데이터 68% 증가에도 성능 하락 → 단순 확장 무효, 도메인 적응 필요
- `load_xjtu_rtf()` 함수는 코드에 잔존 (향후 도메인 적응 실험 시 활용 가능)
...
- 합성 데이터(SMOTE 등) 활용하여 소수 클래스 보강 검토 가능
