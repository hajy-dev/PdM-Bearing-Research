    ...
    for b_dir in tqdm(bearing_dirs, desc="[FEMTO]", unit="bearing"):
        ...
        for fp in csv_files:
            try:
                ...
                file_rms.append(compute_rms(signal))
            except Exception:
                file_rms.append(0.0)
        ...
        failure_idx = len(file_rms) - 1
        for idx, rms in enumerate(file_rms):
            if rms > FAILURE_THRESHOLD_G:
                failure_idx = idx
                break
...
def load_kaist() -> pd.DataFrame:
    ...
    for fp in tqdm(csv_list, desc="[KAIST]", unit="file"):
        try:
            ...
            file_stats.append({
                "filepath": fp,
                "vibration_rms": compute_rms(combined),
        ...
        for fault_ch in info["fault_bearing"]:
            ...
            for fp_name in tqdm(files, desc=f"[IMS] {test_name} B{fault_ch}", unit="file", leave=False):
                ...
                try:
                    ...
                    if info["n_channels"] == 8:
                        # 8채널: bearing N의 2축 = 컬럼 2*N, 2*N+1
                        ch1 = data[:, fault_ch * 2]
                    ...
                    file_rms.append(compute_rms(combined))
                except Exception:
                    file_rms.append(0.0)
            ...
            for idx in range(total):
                ...
                records.append({
                    ...
                    "bearing_id": bearing_id,
                    "file_idx": idx,
                    "vibration_rms": file_rms[idx] if idx < len(file_rms) else 0.0,
                    "rul_ratio": rul_ratio,
                    "total_files": total,
                    "has_temp": False,
                })
    ...
    for bid in sorted(bearing_parts.keys()):
        ...
        for fp in tqdm(all_mat_files, desc=f"[Zenodo] {bid}", unit="file", leave=False):
            try:
                ...
                # accHorizRear_A 사용 (테스트 베어링에 가까운 센서)
                sig = mat['accHorizRear_A'].flatten()
                # V → g 변환
                sig_g = sig * ZENODO_V_TO_G
                file_rms.append(compute_rms(sig_g))
            except Exception:
                file_rms.append(0.0)

        # 진동 파일 수와 온도 측정 수 맞춤 (min 기준)
        n_use = min(len(file_rms), n_measurements)
