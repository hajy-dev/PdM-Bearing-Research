...
def main():
    ...
    # operating_hours가 있으면 추가
    if 'operating_hours' not in df_all.columns:
        df_all['operating_hours'] = df_all.groupby('bearing_id')['file_idx'].transform(
            lambda x: x / max(x.max(), 1)
        )
    common_features.append('operating_hours')
