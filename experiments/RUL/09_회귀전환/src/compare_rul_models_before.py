...
from feature_engineering import (add_trend_features, compute_health_index,
                                  add_severity_labels_hi, SEVERITY_NAMES)
...
def compute_sample_weights(y):
    """클래스 불균형 보정을 위한 sample_weight 계산"""
...
def optuna_xgboost(X, y, groups, n_trials=80):
    """XGBoost 하이퍼파라미터 Optuna 최적화 (과적합 억제)"""
    def objective(trial):
        params = {
...
def optuna_lgbm(X, y, groups, n_trials=80):
    """LightGBM 하이퍼파라미터 Optuna 최적화 (과적합 억제)"""
    def objective(trial):
        params = {
            'n_estimators': trial.suggest_int('n_estimators', 100, 500),
...
def optuna_rf(X, y, groups, n_trials=60):
    """RandomForest 하이퍼파라미터 Optuna 최적화 (과적합 억제)"""
    def objective(trial):
        params = {
            'n_estimators': trial.suggest_int('n_estimators', 100, 500),
...
def main():
    ...
    # 4등급 라벨 생성 (HI 기반)
    df_all = add_severity_labels_hi(df_all)
    ...
    best_rf_a = optuna_rf(X_a, y_a, groups_a, n_trials=60)
    ...
    results_a['RandomForest'] = evaluate_model(
        lambda: RandomForestClassifier(**best_rf_a, random_state=42, n_jobs=-1),
        X_a, y_a, groups_a)
