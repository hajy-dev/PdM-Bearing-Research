...
def hi_to_severity(hi: float) -> int:
    """
    Health Index를 위험도 4등급으로 변환한다.

      > 0.7       → 0 (LOW)      — 정상 상태
      0.4 ~ 0.7   → 1 (MEDIUM)   — 초기 열화
      0.15 ~ 0.4  → 2 (HIGH)     — 진행된 열화
      ≤ 0.15      → 3 (CRITICAL) — 고장 임박
    """
    if hi > 0.7:
        return 0
    elif hi > 0.4:
...
def add_severity_labels_hi(df: pd.DataFrame) -> pd.DataFrame:
    """Health Index에서 4등급 라벨을 생성한다."""
    df = df.copy()
    df['severity'] = df['health_index'].apply(hi_to_severity)
    df['severity_name'] = df['severity'].map(lambda x: SEVERITY_NAMES[x])
    return df
