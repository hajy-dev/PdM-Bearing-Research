...
NORMAL_THRESHOLD = 0.7      # health_index > 0.7 = 정상
ANOMALY_THRESHOLD = 0.4     # health_index ≤ 0.4 = 이상
...
def main():
    ...
    df = compute_health_index(df, hi_features)

    features = [
        # HI 소스 피처 제거 (vibration_rms, short_trend, rolling_trend) — leakage 검증
        # 시간 도메인 통계
    ...
    for feat in features:
        df[feat] = df.groupby('bearing_id')[feat].transform(
            lambda x: (x - x.mean()) / max(x.std(), 1e-10)
        )

    section("Step 3. 정상/이상 분리")
    df_n = df[df['health_index'] > NORMAL_THRESHOLD]
    df_a = df[df['health_index'] <= ANOMALY_THRESHOLD]
